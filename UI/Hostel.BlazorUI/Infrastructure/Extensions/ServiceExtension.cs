...
namespace Hostel.BlazorUI.Infrastructure.Extensions
{
    ...
    public static class ServiceExtension
    {
        ...
        /// <returns></returns>
        public static IHttpClientBuilder AddApi<IInterface, IClient>(this IServiceCollection services,string baseAddress, string address)
            where IInterface : class where IClient : class, IInterface => services
            .AddHttpClient<IInterface, IClient>(
                (host, client) => client.BaseAddress = new($"{baseAddress}/{address}"));
    }
}
