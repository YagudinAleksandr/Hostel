...
namespace Hostel.SUApplication.Tests.Commands
{
    ...
    public class UpdateUserEmailCommandTests
    {
        ...
        public async Task Handle_ShouldReturnSuccess_WhenUserUpdateEmailSuccessfully()
        {
            ...
            _mockRepo.Setup(r => r.IsEmailExistsAsync("test@example.com", userId, default)).ReturnsAsync(false);
        ...
        public async Task Handle_ShouldReturnException_WhenUserUpdateEmailExists()
        {
            ...
            _mockRepo.Setup(r => r.IsEmailExistsAsync("test2@example.com", It.IsAny<Guid>(), default)).ReturnsAsync(true);
            _mockMapper.Setup(m => m.Map<UserResponse>(It.IsAny<User>())).Returns(new UserResponse
            {
                Id = userId,
